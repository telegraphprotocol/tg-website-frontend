export const UPDATES_COOKIE_NAME = "updates_session"
...
export async function signSession(expiresAt: number): Promise<string> {
  const secret = process.env.UPDATES_AUTH_SECRET
  if (!secret) throw new Error("UPDATES_AUTH_SECRET is not set")
...
export async function verifyPassword(input: string): Promise<boolean> {
  const expected = process.env.UPDATES_PASSWORD
...
export const UPDATES_SESSION_MAX_AGE_SECONDS = SESSION_DURATION_MS / 1000
