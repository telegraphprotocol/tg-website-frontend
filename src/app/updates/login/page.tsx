...
export default function UpdatesLoginPage() {
  return (
    <Suspense>
      <UpdatesLoginForm />
    </Suspense>
...
function UpdatesLoginForm() {
  const searchParams = useSearchParams()
  const from = searchParams.get("from") ?? "/updates"
  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-4">
      <div className="w-full max-w-md space-y-8 border rounded-2xl p-8">
        <div>
          <h2 className="text-2xl font-semibold text-center">Updates</h2>
          <p className="mt-2 text-center text-sm text-muted-foreground">
            Enter the password to view updates
          </p>
        </div>
